...
class ApiService {
  ...
  // Pulse API (AI Co-Pilot)
  async getPulseStale(days = 7) {
    return this.request(`/pulse/stale?days=${days}`)
  }

  async updateHealthScores() {
    return this.request('/pulse/update-scores', { method: 'POST' })
  }
  ...
  async setOptIn(matchId, role, value) {
    return this.request(`/matches/opt-in/${matchId}/${role}`, {
      method: 'PUT',
