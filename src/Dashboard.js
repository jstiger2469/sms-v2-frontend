...
const Dashboard = () => {
  ...
  // const [totalMessagesBySenderType, settotalMessagesBySenderType] = useState(0);
  const [averageResponseTime, setAverageResponseTime] = useState(0);
  const [messagesByPeriod, setMessagesByPeriod] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await DashboardDataService.getTotalMessages();
        const senders = await DashboardDataService.getMessagesBySenderType();

        const messages = response.data;
        console.log(messages)
        console.log(senders)
        // Total messages
        setTotalMessages(messages.totalMessages);

        console.log('Total Messages:', totalMessages);


        const averageResponseTime = await DashboardDataService.getAverageResponseTime();

        console.log(averageResponseTime)

        // Calculate average response time
        const responseTimes = messages.totalMessages.map((message) => {
          const sentTime = new Date(message.timestamp);
          const matchStartTime = new Date(message.match.createdAt);
          return sentTime - matchStartTime;
        });

        const avgTime =
          responseTimes.length > 0
            ? responseTimes.reduce((acc, cur) => acc + cur, 0) /
              responseTimes.length
            : 0;

        setAverageResponseTime(Math.round(avgTime / 1000 / 60)); // In minutes

        // Aggregate messages by time period (e.g., past week)
        const now = new Date();
        ...
        setMessagesByPeriod(filteredMessages.length);
  ...
  // Chart Data
  const barData = {
    labels: ["Past Week", "All Time"],
    datasets: [
      {
        label: "Messages",
        data: [messagesByPeriod, totalMessages],
        backgroundColor: ["#4A90E2", "#50E3C2"],
        borderColor: ["#4178A7", "#41A79E"],
        borderWidth: 1,
      },
    ],
  };
  ...
  const lineData = {
    labels: ["1", "2", "3", "4", "5", "6", "7"], // Days of the week
    datasets: [
      {
...
<div className="rounded-sm border border-stroke bg-white px-7 py-6 shadow-default dark:border-strokedark dark:bg-boxdark">
    <div className="flex h-11.5 w-11.5 items-center justify-center rounded-full bg-meta-2 dark:bg-meta-4">
        ...
        <div className="p-4 border rounded shadow-sm bg-white">
          ...
          <p className="text-2xl font-bold">{averageResponseTime} mins</p>
        </div>
      </div>

      {/* <div className="mb-6">
        <h3 className="font-medium text-lg mb-2">Messages Chart</h3>
        <Bar data={barData} options={{ responsive: true, maintainAspectRatio: false }} />
      </div>

      <div className="mb-6">
        <h3 className="font-medium text-lg mb-2">Messages Per Day</h3>
