...
const ResponseRateBySenderTypeChart = () => {
  ...
  useEffect(() => {
    const fetchResponseRateData = async () => {
      try {
        const response = await DashboardDataService.getResponseRateBySenderType();
        const data = response.data;
        // Only keep Mentor and Student, and order Mentor first, Student second
        const filtered = data.filter(item => item.senderType === 'Mentor' || item.senderType === 'Student');
        const sorted = filtered.sort((a, b) => (a.senderType === 'Mentor' ? -1 : 1));
        const labels = sorted.map((item) => item.senderType);
        const responseRates = sorted.map((item) => item.responseRate);
        setChartData({
